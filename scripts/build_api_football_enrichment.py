...
def page_cache_path(season: int, page: int) -> pathlib.Path:
...
def load_or_fetch_page(
    *,
    ...
    path = page_cache_path(season, page)
...
def fetch_season(
    *,
    ...
    return records, pages_to_read, cached_pages
    ...
    for item in records:
        ...
        for statistics in item.get("statistics") or []:
            ...
            row = {
                ...
                "pass_accuracy_pct": to_number(passes.get("accuracy")),
...
- Pages served from cache: {cached}
...
Do not add an API-Football field to similarity until it has at least 80% non-null
coverage for the relevant season and position. Zero is a valid football value;
missing values remain null so unavailable provider data is not mistaken for zero.
...
def main() -> int:
    ...
    parser.add_argument("--seasons", default="2025", help="Comma-separated start years, for example 2024,2025")
    ...
    parser.add_argument("--max-pages", type=int, help="Optional safety limit for a coverage test")
    ...
    quota = DailyQuota(LEDGER_PATH, args.daily_budget)
    ...
    try:
        for season in seasons:
            records, pages, cached_pages = fetch_season(
                season=season,
            ...
            page_summary.append({"season": season, "pages": pages, "cached_pages": cached_pages})
            print(
                f"{season_label(season)}: {len(records)} provider rows from {pages} page(s) "
                f"({cached_pages} cached)"
            )
